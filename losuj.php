<?php
  ...
  switch($_GET['step']){
    case '1':
      ...
      if($wpis->author!=null){
        ...
        //jest jakies nieprawidlowe, albo ktos sie glupio bawi aplikacja
          ...
          echo '<p class="error">Niestety źle wpisałeś adres wpisu :/. Aby uzyskać adres wpisu, kliknij na znaczek #, który znajduje się obok liczby plusów.</p>';
        }
      }
      //wpis ma pustego autora, wiec prawdopodobnie nie istnieje wpis o takim id
      else{
        echo '<p class="error">Niestety nie znalazłem takiego wpisu :/. Może autor go już usunął? Sprawdź jeszcze raz czy wkleiłeś poprawny adres!</p>';      
      }
    ...
    case '2':
      ...
      if($wpis->voters==null){
        echo '<p class="error">Niestety nikt nie plusowal tego wpisu :/</p>';
        break;
      } 

      //sprawdzenie roznicy czasow
      if($czas_od>$czas_do){
        echo '<p class="error">Godzina zakończenia losowania jest wcześniejsza niż godzina rozpoczęcia!</p>';
        break;
      }
      ...
      if((time()-120)<$czas_do){
        echo '<p class="error">Musisz poczekać z losowaniem do godziny jego zakończenia. Jeśli nadeszła już ta godzina, odczekaj jeszcze około 2 minuty (ewentualna różnica czasu na serwerach dla bezpieczeństwa).</p>';
        break;
      }
      ...
      if($ilu_bierze_udzial==1){
         echo '<p class="error">Tylko jedna osoba jest uprawniona do udziału w losowaniu (jedyny plus w danym zakresie czasu). Sens losowania stoi więc pod sporym znakiem zapytania :).</p>';
         break;
      }
      else if($ilu_bierze_udzial==0){
         echo '<p class="error">Niestety nikt nie dał plusa w tym zakresie czasu :/. Musisz rozszerzyć ramy czasowe.</p>';
         break;
      }
